...
export const RangePresetButton = ({ value, className, children, ...props }: RangePresetButtonProps) => {
    ...
    return (
        <button
            {...props}
            className={cx(
                "cursor-pointer rounded-md px-3 py-2 text-left text-sm font-medium transition duration-100 ease-linear focus-visible:outline-2 focus-visible:outline-offset-2",
                isSelected
                    ? "bg-[var(--bg-hover)] text-[var(--accent-ink)] font-semibold"
            ...
            >
                <div className="flex flex-col gap-3 px-6 py-5 md:gap-2">
                    <header className={cx("relative flex items-center", visibleDurationMonths > 1 ? "justify-start" : "justify-between")}>
                        ...
                        <h2 className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-sm font-semibold text-[var(--text-primary)]">
                        ...
                        <AriaCalendarGridHeader>
                            {(day) => (
                                <AriaCalendarHeaderCell className="border-b-4 border-transparent p-0">
                                    <div className="flex size-10 items-center justify-center text-sm font-medium text-[var(--text-muted)]">{day.slice(0, 2)}</div>
                ...
                {visibleDurationMonths > 1 && (
                    <div className="flex flex-col gap-3 border-l border-[var(--border-light)] dark:border-[#242424] px-6 py-5">
