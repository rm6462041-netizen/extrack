...
export const Slider = ({
    ...
    return (
        <AriaSlider {...rest} {...{ minValue, maxValue }} formatOptions={formatOptions ?? defaultFormatOptions}>
            <AriaLabel />
            <AriaSliderTrack className="relative h-6 w-full touch-none select-none cursor-pointer">
                {({ state: { values, getThumbValue, getThumbPercent, getFormattedValue } }) => {
                    ...
                    return (
                        <>
                            <span
                                className={cx("absolute top-1/2 h-2 w-full -translate-y-1/2 rounded-full", trackClassName || "bg-quaternary")}
                                style={trackStyle}
                            />
                            {showFillTrack && (
                                <span
                                    className={cx(
                                        "absolute top-1/2 h-2 w-full -translate-y-1/2 rounded-full transition-none",
                                        fillClassName || "bg-brand-solid"
                                    )}
                                    style={{
                                        left: `${left * 100}%`,
                                        width: `${width * 100}%`,
                                ...
                                return (
                                    <AriaSliderThumb
                                        ...
                                        className={({ isFocusVisible, isDragging }) =>
                                            cx(
                                                "top-1/2 box-border size-6 cursor-grab rounded-full bg-slider-handle-bg shadow-md ring-2 ring-slider-handle-border ring-inset transition-none",
