...
export const ToggleBase = ({ className, isHovered, isDisabled, isFocusVisible, isSelected, slim, size = "sm" }: ToggleBaseProps) => {
    const styles = {
        default: {
            sm: {
                root: "h-5 w-9 p-0.5",
                switch: cx("size-4", isSelected ? "translate-x-4" : "translate-x-0"),
            },
            md: {
                root: "h-6 w-11 p-0.5",
                switch: cx("size-5", isSelected ? "translate-x-5" : "translate-x-0"),
            },
        },
        slim: {
            sm: {
    ...
    return (
        <div
            className={cx(
                "relative cursor-pointer rounded-full bg-tertiary ring-[0.5px] ring-secondary outline-focus-ring transition-colors duration-250 ease-in-out ring-inset",
        ...
        >
            <div
                className={cx(
                    "rounded-full bg-fg-white shadow-sm transition-transform duration-250 ease-in-out transform-gpu",
